      ...
      <div class="container" v-if="show">
        <article v-for="(pokemon, index) in pokemons" :key="index">
          ...
          >
            ...
            <div class="infos">
              <img
                :src="require(`../assets/img/${pokemon.numero}.png`)"
                :alt="pokemon.nom"
                class="pics"
              />
              <div class="infos-powers">
                ...
                <p>
                  Attaque : <span>{{ pokemon.attaque }}</span>
                </p>
                <p>
                  Défense : <span>{{ pokemon.defense }}</span>
                </p>
                <p>
                  Attaque Spé : <span>{{ pokemon.attaque_spe }}</span>
                </p>
                <p>
                  Défense Spé : <span>{{ pokemon.defense_spe }}</span>
                </p>
                <p>
                  Vitesse : <span>{{ pokemon.vitesse }}</span>
                </p>
              </div>
            </div>
          </router-link>
  ...
  methods: {
    getPokemonsByTypes: async function () {
      try {
        ...
        const response = await axios.get(`${this.base_url}/pokemon-by-type/${typeId}`);
    ...
    getOneType: async function () {
      try {
        const typeId = parseInt(this.$route.params.typeId, 10);
        const response = await axios.get(`${this.base_url}/type/${typeId}`);
